...
class Core(cmd.Cmd):
    ...
    def __lookingForGadgets(self, print_gadgets):
        ...
        if print_gadgets:
            ...
            for gadget in self.__gadgets:
                ...
                print(("0x%08x" % vaddr if arch_mode == CS_MODE_32 else "0x%016x" % vaddr) + " : %s" % insts)
            ...
            try:
                for section in sections:
                    ...
                    for ref in allRef:
                        ...
                        rangeS = int(self.__options.range.split('-')[0], 16)
                        rangeE = int(self.__options.range.split('-')[1], 16)
                        if (rangeS == rangeE == 0) or (rangeS <= vaddr <= rangeE):
                            print(
                                ("0x%08x" % vaddr if arch == CS_MODE_32 else "0x%016x" % vaddr) + " : '%c'" % (
                                    char))
                            raise
            except:
                pass
    ...
    def analyze(self, exec_sections, print_gadgets=False, print_process=False):
        self.__offset = 0
        ...
        code = ROPMaker(self.__arch, self.__arch_mode, self.__format, self.__gadgets, self.__offset,
                        self.__data_section).generate(print_process)
        return code
